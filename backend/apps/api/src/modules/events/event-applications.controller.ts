...
import { SpotNotificationResponseDto } from "@api/modules/events/dto/spot-notification-response.dto";
...
${application?.invoiceAddress?.country}`,
  ...
  /**
   * Send spot notification emails to all participants who have a spot assigned
   */
  @ApiOkResponse({
    type: SpotNotificationResponseDto,
    description: "Number of emails sent",
  })
  @ApiNotFoundResponse({ description: "Event not found" })
  @ApiBearerAuth()
  @UseGuards(CookieGuard)
  @Post(":id/spot-notifications")
  async sendSpotNotifications(
    @CurrentUser() currentUser: User,
    ...
    if (
      ...
      throw new ForbiddenException(
        "You don't have permission to send spot notifications",
      );
    }
    ...
    const applications =
      await this.eventApplicationsService.findByEventIdDetailed(eventId);
    const withSpot = applications.filter((app) => app.spotType !== null);

    const webDomain =
      this.configService.get<string>("WEB_DOMAIN") ?? "http://localhost:3000";
    let sent = 0;

    for (const application of withSpot) {
      try {
        await this.mailService.sendSpotNotification({
          email: application.user.email,
          name: `${application.user.firstName} ${application.user.lastName}`,
          eventName: event.title,
          eventSince: dayMonthYear(event.since),
          eventUntil: dayMonthYear(event.until),
          spotName: application.spotType.name,
          spotPrice: application.spotType.price,
          spotCurrency: application.spotType.currency,
          eventLink: `${webDomain}/event/${eventId}`,
        });
        sent++;
      } catch (error) {
        console.error(
          `Failed to send spot notification to ${application.user.email}`,
          error,
        );
      }
    }
