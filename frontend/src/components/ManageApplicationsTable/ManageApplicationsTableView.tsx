...
interface ManageApplicationsTableViewProps {
  eventId: number;
  eventSpotsList: EventSpotSimple[];
  applications: EventApplicationDetailedWithApplications[];
}

const ManageApplicationsTableView = ({ eventId, eventSpotsList, applications }: ManageApplicationsTableViewProps) => {
  const router = useRouter();

  // Spots and applications come from the server component - re-run it after structural changes
  const handleRefetchApplications = () => {
    router.refresh();
  };

  // Local copy so spot changes update a single row without reloading the page. Reset when the server sends new data.
  ...
  const deleteEventSpotMutation = useDeleteEventSpot({
    mutation: {
      onSuccess: handleRefetchApplications,
    },
  });
  ...
  const spots: ComboboxData = useMemo(() => {
    return eventSpotsList.map((spot) => ({
  ...
  const setApplicationSpot = (
    applicationId: number,
    spotType: EventApplicationDetailedWithApplications["spotType"],
  ) => {
    setRows((current) =>
      current.map((application) => (application.id === applicationId ? { ...application, spotType } : application)),
    );
  };

  // Optimistic - show the new spot immediately, revert the row when the request fails
  const handleChangeApplicationSpot = (
    application: EventApplicationDetailedWithApplications,
    spotId: number | null,
  ) => {
    const previousSpot = application.spotType;
    setApplicationSpot(application.id, eventSpotsList.find((spot) => spot.id === spotId) ?? null);

    updateApplicationSpotMutation.mutate(
      { applicationId: application.id, data: { spotId } },
      { onError: () => setApplicationSpot(application.id, previousSpot) },
    );
  };
  ...
  return (
    <>
      ...
      <DataTable columns={columns} data={rows} emptyMessage="No applications found." />
...
export default ManageApplicationsTableView;
